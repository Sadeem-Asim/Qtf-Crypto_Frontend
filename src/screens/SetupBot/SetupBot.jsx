...
const getInitialState = (
  { setting, user, isActive, coin, availableBalance, investment, exchange },
  data
) => {
  if (setting) {
...
function SetupBot() {
  ...
  useEffect(() => {
    if (routeState && user) {
      setData(getInitialState({ ...routeState }, data));
    }
  }, [routeState]);
  ...
  useEffect(() => {
    (async () => {
      const conditionFetchRSI =
        data?.indicator === INDICATORS.rsi &&
        botSetting?.rsi?.time &&
        data.coin;
      // console.log({i: data?.indicator, t: botSetting?.rsi?.time, c: data?.coin});
      if (conditionFetchRSI) await refetchRSIValue();
    })();
  ...
  const { mutate: setupBot, isLoading } = useMutation(
    "saveBotSettings",
    apis.configureBotSetting,
    {
  ...
  const onChangeBotSettingHandler = (event) => {
    ...
    if (name === "botStatus") {
      /* Handling bot switch */ setBotSetting((prevState) => ({
        ...prevState,
        [type]: { ...prevState[type], isActive: !prevState[type]["isActive"] },
      }));
    } else if (name === "time") {
      /* Handling RSI values */ setBotSetting((prevState) => ({
        ...prevState,
        [type]: {
          ...prevState[type],
          time: getRSIFormatTime(value, routeState?.exchange),
        },
      }));
    } else {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (availableBalance > bot.investment) {
      if (total > availableBalance) {
        toast.warn(
          `Investment (${total}) is greater than available balance (${availableBalance})`
        );
        return;
      }
    } else {
      if (total > bot.investment) {
        toast.warn(
          `Investment(${total}) is greater than Total investment (${bot.investment})`
        );
        return;
      }
    }
