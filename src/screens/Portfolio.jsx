...
function Portfolio() {
  ...
  const { isLoading, refetch: refetchOrders } = useQuery(
    "activity",
    apis.activityRecords,
    {
  ...
  const handleAPIConfiguration = (id) =>
    navigate("/api-configuration", { state: { userId: id } });
  const updateBotStatus = async ({ isActive, _id }) =>
    await mutateAsync({ _id, body: { isActive: !isActive } });
              ...
              <table className="table custom-table">
                ...
                <tbody>
                  {users.map((user, index) => {
                    if (user.bots?.length === 0) return null;
                    return (
                      <PortfolioTableRow
                        counter={index + 1}
                        key={index}
