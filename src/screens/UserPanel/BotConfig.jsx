...
function BotConfig() {
  ...
  const { data: kucoins, balance: _balance } = useSelector(
    (store) => store.kucoin
  );
  ...
  useEffect(() => {
    if (kucoins || botData.exchange) {
      const value =
        botData.exchange === EXCHANGES.binance
          ? EXCHANGES.binance
  ...
  const onChangeHandler = (e) => {
    ...
    if (id === "stop_at" && coin) {
      ...
      if (Number(value) > lossLimit) {
        toast.warning(`Stop Loss can not be greater than ${lossLimit}`, {
          toastId: id,
        });
      ...
      if (Number(value) > currentBalance) {
        if (currentBalance === 0)
          return toast.warning(`Your account balance is ${currentBalance}`, {
            toastId: id,
          });
        toast.warning(`Investment can not be greater than ${currentBalance}`, {
          toastId: id,
        });
  ...
  const typesHandler = (key, type) => {
    const RISKS = { LOW: 7, MODERATE: 15, HIGH: 45 };
  ...
  const getSelectedExchangeBalance = () => {
    if (botData?.exchange === EXCHANGES.kuCoin) return _balance;
    else if (botData.exchange === EXCHANGES.binance) return balance;
    else return 0;
  };
          ...
          <Form onSubmit={onSubmitHandler}>
            ...
            <div className="mt-5">
              <Container fluid>
                  ...
                  {RISKS.map((risk, index) => {
                    ...
                    return (
                      <Col lg={4} md={6} key={index}>
                        <div
                          className={`bot-box pointer ${
                            botData.risk === risk ? "active" : ""
                          }`}
                        ...
                        >
                          ...
                          <h4 className="heading">
                            {_.capitalize(risk)} Risk{" "}
                          </h4>
                          <h4 className="heading">
                            {" "}
                            Minimum {percentage} Stop Loss{" "}
                          </h4>
                        </div>
                      </Col>
                    );
                ...
                <div className="bot-tabs mt-5"></div>
                <Row>
                  <Col md={6}>
                ...
                <div className="bot-tabs mt-4"></div>
                  ...
                  <div className="investment-main">
                    <div className="low-section">
                      ...
                      <svg
                        ...
                        viewBox="0 0 37 47"
                        fill={
                          textColorClass(getSelectedExchange("change")) &&
                          textColorClass(getSelectedExchange("change"))?.split(
                            "-"
                          )[1]
                        }
                      >
