...
import {
  getExchangeCoinsValues,
  getTradingWidgetSymbol,
  textColorClass,
} from "utils";
...
function Dashboard() {
  ...
  const {
    ...
    data: adminData,
    refetch,
  } = useQuery(["getAllProfitAndLossAdmin"], () =>
    apis.profitLossAccountDetailsAdmin(tab)
  );
          ...
          <Container fluid>
              ...
              <ul className="justify-content-start">
                <li>
                  <a
                    ...
                    href={"#!"}
                    onClick={() => setTab(EXCHANGES.binance)}
                  >
                    Binance
                  </a>
                </li>
                {/* <li>
                  <a
                    className={tab === "kuCoin" && "active"}
                    href={"#!"}
                    onClick={() => setTab(EXCHANGES.kuCoin)}
                  >
                    KuCoin
                  </a>
                </li> */}
            ...
            <Row className="gy-5">
              {/* {_.values(getExchangeCoinsValues(tab, coins, kucoins)).length > 0
                ? _.values(getExchangeCoinsValues(tab, coins, kucoins)).map(
                    (_coin) => {
                      const symbol = _coin.symbol?.toUpperCase();
                      const change = _coin.change;
                      const active_class = symbol === coin ? "box-active" : "";
                      const change_class =
                        change > 0
                  ...
                  <div className="normal-box">
                    ...
                    <div className="inner-main">
                      ...
                      <div className="inner">
                        ...
                        <span
                          className={textColorClass(record?.todayProfitPrice)}
                        >
                          {record?.todayProfitPrice}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="normal-box mt-4">
                    ...
                    <div className="flex-text-between">
                      <h3>
                        Daily Profit (
                        {
                          record?.dailyProfitChart?.[
                            profitDays?.dailyProfitDays
                          ]
                        }
                        )
                      </h3>
